...
#galaxy_sample = "BOSS_North" # goes with BN
galaxy_sample = "2MPZ" # both ACT maps
#galaxy_sample = "SDSS_L43D" # "SDSS_L61D" # "SDSS_L43" # "SDSS_L61" # "SDSS_L79"
...
if cmb_sample == "ACT_BN":
    ...
    fn = "../cmb_tests/cmb_data/tilec_single_tile_D56_comptony_map_v1.2.0_joint.fits" # D56
...
if cmb_sample == "ACT_BN":
    ...
    pass
print("percentage not masked = ", np.sum(msk)*100./(msk.shape[0]*msk.shape[1])) # 58%
...
D_A_zmed = Cosmo.luminosity_distance(zmed)/(1.+zmed)**2 # Mpc
...
for i in range(len(RA)):
    ...
    if (stampMap is None): print("None, ra, dec = ", ra, dec); continue
    
    stack += stampMap
    count += 1
stack /= count
print("nonzero objects percentage = ", count*100./np.sum(choice))
plt.imshow(stack)
...
enplot.write(f"figs/tSZ_{galaxy_sample}_{cmb_sample:s}", plots)
